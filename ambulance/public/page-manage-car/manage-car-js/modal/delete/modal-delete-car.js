...
import { removeTableRow } from '/table-car-update-row.js';
...
export async function deleteCar(id) {
    try {
        ...
        // ดึงชื่อรายการ
        const itemLicense = getItemLicense(id);
        const confirmed = await confirmDelete(itemLicense);
        if (!confirmed) return;
 ...
 */
function getItemLicense(id) {
    const row = document.querySelector(`button[onclick="deleteItem(${id})"]`)?.closest('tr');
    const licenseCell = row?.querySelector('.table-name');
    return licenseCell ? licenseCell.textContent.trim() : 'รายการนี้';
}

/**
 * ยืนยันการลบ
 */
async function confirmDelete(itemLicense) {
    const result = await Swal.fire({
        title: 'ยืนยันการลบ',
        html: `คุณต้องการลบรายการ<br><strong style="color: #dc3545;">"${itemLicense}"</strong><br>หรือไม่?`,
...
async function performDelete(id) {
    ...
    try {
        const response = await fetch(API_ENDPOINTS.MANAGE_CAR.DELETE, {
            method: 'DELETE',
